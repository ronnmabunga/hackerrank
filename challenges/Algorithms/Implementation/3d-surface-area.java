...
public class Solution {
    ...
    static int surfaceArea(int[][] A) {
        ...
        */
        ...
        for(int i = 1; i < L-1; i ++){
            for(int j = 1; j < W-1; j ++){
                a = A[i-1][j]; b = A[i][j-1]; c = A[i][j]; d = A[i][j+1]; e = A[i+1][j];
        ...
        // NorthWest Corner
        c = A[0][0]; d = A[0][1]; e = A[1][0]; 
        total += 2*c+(d>c?0:c-d)+(e>c?0:c-e);
        // NorthEast Corner
        b = A[0][W-2]; c = A[0][W-1]; e = A[1][W-1]; 
        total += 2*c+(b>c?0:c-b)+(e>c?0:c-e);
        // SouthWest Corner
        a = A[L-2][0]; c = A[L-1][0]; d = A[L-1][1];
        total += 2*c+(a>c?0:c-a)+(d>c?0:c-d);
        // SouthEast Corner
        a = A[L-2][W-1]; b = A[L-1][W-2]; c = A[L-1][W-1];
